        ...
        pushData: function (list) {
            ...
            for (i = 0; i < list.length; i++) {
                if(list[i] && list[i].url) {
                    ...
                    var $btns = $('<div class="file-panel"><span class="cancel">' + lang.uploadDelete + '</span></div>').appendTo(item);
                    $btns.on('click', 'span', function () {
                        var index = $(this).index();
                        var that = this;
                        switch (index){
                            case 0:
                                ajax.request(editor.getActionUrl(editor.getOpt('destoryActionName')), {
                                    timeout: 100000,
                                    data: utils.extend({
                                            path: $(that.parentNode).prev('span').prev('img').attr('_src')
                                        },
                                        editor.queryCommandValue('serverparam')),
                                    method: 'delete',
                                    onsuccess: function (r) {
                                        try {
                                            ...
                                            if (json.state == 'SUCCESS') {
                                                $(that.parentNode).prev('span').prev('img').remove();
                                            } else {
                                                alert(json.msg);
                                            }
                                        } catch (e) {}
