<div class="panel-heading">
    admin {{ Lang::get('Admin::'.$_current_controller.'.edit')  }}
</div>
...
@if( isset( $config['form']['show'] ) && $config['form']['show'] )
    ...
    @else
    {{ Form::$config['form']['type']($field, Input::get($field) ?: $item->$field ?: (isset($config['form']['value']) ? $config['form']['value'] : ''), array( 'placeholder' => Lang::get('Admin::'.$_current_controller.'.'.$field) ) + (isset($config['form']['attr']) ? $config['form']['attr'] : array()) ) }}
    @endif
...
@endif
@endforeach
{{ Form::submit(Lang::get('Admin::'.$_current_controller.'.edit'), array('class' => 'btn btn-primary pull-right')) }}
{{ Form::close() }}
@else
